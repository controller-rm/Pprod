...
from reports.pdf_pprod import gerar_pdf_pprod
...
from database import (
    get_connection_softdib,
    get_connection_realfix
)
...
def main():
    ...
    with col_botao:
        st.write("")
        st.write("")
        if st.button("🔄 Atualizar", use_container_width=True, key="btn_atualizar_app_pprod"):
            st.cache_data.clear()
            st.rerun()
    ...
    def carregar_pprod():
        ...
        try:
            ...
            df_banco["Nw_Data"] = pd.to_datetime(
                df_banco["Nw_Data"],
                errors="coerce"
            ).dt.strftime("%d/%m/%Y").fillna("")

            df_banco["Dt.Subida"] = pd.to_datetime(
                df_banco["Dt.Subida"],
                errors="coerce"
            ).dt.strftime("%d/%m/%Y").fillna("")

            df_banco = df_banco.drop_duplicates(
                subset=["Nro_OF", "Produto OF"],
                keep="last"
            )
            ...
            st.error(f"Erro ao carregar dados da tabela pprod: {e}")
            return pd.DataFrame(columns=colunas)


    def salvar_pprod(df_tabela):
        df_save = df_tabela.copy()

        if "Nro OF" in df_save.columns:
            df_save = df_save.rename(columns={"Nro OF": "Nro_OF"})

        colunas_saida = ["Nw_Data", "Nro_OF", "Produto OF", "Prioridade", "Dt.Subida", "Remover"]

        for col in colunas_saida:
            if col not in df_save.columns:
                df_save[col] = False if col == "Remover" else ""
        ...
        try:
            ...
            for _, row in df_save.iterrows():
                nro_of = row["Nro_OF"]
                produto_of = row["Produto OF"]
                prioridade = row["Prioridade"]
                nw_data = row["Nw_Data"]
                dt_subida = row["Dt.Subida"]
                remover = bool(row["Remover"])

                if not nro_of or not produto_of:
                    continue
                ...
                if remover or campos_vazios:
                    cursor.execute(sql_delete, (nro_of, produto_of))
                    continue

                nw_data_mysql = None
                dt_subida_mysql = None

                if nw_data:
                    nw_data_mysql = datetime.strptime(
                        nw_data,
                        "%d/%m/%Y"
                    ).strftime("%Y-%m-%d")

                if dt_subida:
                    dt_subida_mysql = datetime.strptime(
                        dt_subida,
                        "%d/%m/%Y"
                    ).strftime("%Y-%m-%d")
            ...
            st.error(f"Erro ao salvar dados na tabela pprod: {e}")


    def aplicar_pprod(df_principal, df_banco):
    ...
    try:
        ...
        df_banco = carregar_pprod()
        df = aplicar_pprod(df, df_banco)
        ...
        if st.button("💾 Salvar Produção"):
            ...
            if erros:
                ...
                try:
                    salvar_pprod(df_para_salvar)
                    st.session_state["mensagem_salvo"] = "Dados gravados com sucesso na tabela pprod."
                    st.rerun()
                except Exception as e:
                    st.error(f"Erro ao salvar dados na tabela pprod: {e}")

        with st.expander("Visualizar conteúdo atual da tabela pprod"):
            df_banco_atual = carregar_pprod()
            if df_banco_atual.empty:
                st.write("A tabela pprod ainda não possui registros.")
            else:
    ...
    if st.button("📄 Gerar PDF"):
        pdf = gerar_pdf_pprod(
            ...
            df_producao_diaria=df_producao_diaria,
            filtros=filtros,
            logo_path="Controller.png"
        )

        st.download_button(
            "⬇ Baixar PDF",
            pdf,
            file_name="Ordens_Fabricacao_Abertas.pdf",
